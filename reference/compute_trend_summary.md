...
Aggregates data over time into a tidy frame of \`ref_date\`, the
optional grouping column, and \`value\`. Counts rows when
\`measure_col\` is \`NULL\` (headcount) and sums the column otherwise
(wage bill).
