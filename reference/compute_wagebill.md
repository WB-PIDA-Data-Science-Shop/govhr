...
- share_macro:
  ...
  Logical; if `TRUE`, computes wage bill shares relative to macro-fiscal
  aggregates specified in `macro_vars`. Defaults to `FALSE`.

- macro_vars:

  Character vector of macro indicator column names to use as
  denominators when `share_macro = TRUE`. Defaults to
  `c("gdp_lcu", "pexpenditure_lcu", "prevenue_lcu", "taxrevenue_lcu")`.

- drop_na:

  Logical; if `TRUE`, removes `NA` values before aggregation. Defaults
  to `TRUE`.
...
`value` column. When `share_macro = TRUE`, values represent shares (wage
bill / macro aggregate).
