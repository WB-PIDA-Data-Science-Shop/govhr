...
coloured series per group. When \`toggle_growth\` is \`TRUE\` the y-axis
is formatted as a baseline index with a dashed reference line at 100.
...
- .data:
  ...
  \[compute_trend_summary()\] and optionally
  \[govhr::apply_baseline_index()\].
...
- y_label:
  ...
  Character. y-axis label, used when \`toggle_growth\` is \`FALSE\`.
  Default \`"Value"\`.
