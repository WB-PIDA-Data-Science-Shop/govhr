...
# ---------------------------------------------------------------------------
# estimate_movement_baseline — input validation
# ---------------------------------------------------------------------------
test_that("non-data.table input raises error", {
  df <- as.data.frame(two_snap_mover())
  expect_error(
    estimate_movement_baseline(df, group_cols = "paygrade"),
    "data.table"
  )
})
...
test_that("missing group_cols raises error", {
  dt <- two_snap_mover()
  expect_error(
    estimate_movement_baseline(dt, group_cols = character(0)),
    "group_cols"
  )
})
...
test_that("missing columns raises error", {
  dt <- two_snap_mover()
  expect_error(
    estimate_movement_baseline(dt, group_cols = "nonexistent_col"),
    "not found"
  )
})
...
test_that("fewer than 2 snapshots raises error", {
  ...
  expect_error(
    estimate_movement_baseline(dt, group_cols = "paygrade"),
    "2 panel snapshots"
  )
})

# ---------------------------------------------------------------------------
# estimate_movement_baseline — output structure
# ---------------------------------------------------------------------------
test_that("output has expected columns", {
  out <- estimate_movement_baseline(two_snap_mover(), group_cols = "paygrade")
...
# ---------------------------------------------------------------------------
# estimate_movement_baseline — correctness
# ---------------------------------------------------------------------------
...
test_that("three snapshots: movement rates and period columns are correct", {
  ...
  out <- estimate_movement_baseline(dt, group_cols = "paygrade")
...
test_that("multi-column group_cols concatenated with ||", {
  ...
  out <- estimate_movement_baseline(dt, group_cols = c("est_id", "paygrade"))
