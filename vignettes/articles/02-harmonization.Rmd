...
* validate the outputs against the `govhr` Standard Data Dictionary.
...
For this vignette, we apply our tools to data provided by Brazil HRMIS system provided at the contract level. We randomly sample 5 percent of the original data. See the data below:
...
We illustrate the harmonization workflow using this dataset. `bra_hrmis` contains `r nrow(bra_hrmis)` rows and `r ncol(bra_hrmis)` columns, covering covering demographic, employment, establishmental, and payroll information for public sector personnels.
...
The Contract Module is the foundational component of the harmonized payroll dataset. The purpose of this module is to extract and structure all information that is **unique at a contract, reference date level**, as defined in the data dictionary. A “contract record” refers to a personnel–contract relationship at a specific reference period (usually month–year), capturing the contractual characteristics governing the personnel's employment at that time.

In practice, the raw HRMIS data typically mixes variables at different conceptual levels—personnel attributes, position attributes, contract terms, payroll events, and sometimes even one-off administrative transactions. 
...
This clearly shows that `MATRICULA` is the contract ID while `CPF` is possibly the identifier for the personnel, the latter will come in handy during the harmonization of the personnel module. 
...
```{r harmonize-occupation, warning=FALSE, eval = FALSE}
#-----------------------------
...
```{r fake-harmonize-occupation, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}

occup_df <- bra_hrmis[, c("CARREIRA", "CARGO")] |> unique()

occup_df[["CARREIRA"]] <- occup_df[["CARGO"]]

fulloccup_tbl <- arrow::read_parquet(here::here("spielplatz/data/occupations_tbl.parquet"))

occup_df <- 
  fulloccup_tbl |>
  # dplyr::semi_join(occup_df, by = c("CARREIRA", "CARGO")) |>
  distinct() |>
  dplyr::select(CARREIRA, CARGO, occupation_native, occupation_english) |> 
  as.data.table()
...
bra_hrmis_contract_display <- data.table::copy(bra_hrmis_contract)
char_cols <- names(bra_hrmis_contract_display)[sapply(bra_hrmis_contract_display, is.character)]
bra_hrmis_contract_display[, (char_cols) := lapply(.SD, function(x) {
  iconv(x, to = "UTF-8", sub = "")
}), .SDcols = char_cols]

reactable(
  head(bra_hrmis_contract_display, 100)
)
...
The Personnel Module standardizes individual-level information about employees with the public sector. This section demonstrates the harmonization pipeline using the same contract-level Brazil (Alagoas) HRMIS dataset (bra_hrmis) as an example. We will produce a clean, harmonized dataset named bra_hrmis_personnel which conforms to the Personnel Module dictionary in `standard_dictionary.Rmd`.

Let's start with a little exploration. We need to understand how many unique personnel-establishment-refdate combinations are within the data:

```{r creating-unique-personnel-identifiers}
...
This tells us that we have `r personnel_count` unique personnel_id-est_id-refdate combinations in the entire dataset. The goal is to ensure that once all the other variables of the personnel module are added. The size of the personnel module remains the same. As we did, in the contract module, we now add all the derived variables:
...
Finally, we can create the personnel module by taking the set of unique values across the set of personnel modules we have now created within `bra_hrmis`. See the implementation below:

```{r creating-unique-personnel-identifiers2}
...
The Establishment Module extracts, standardizes, and structures information on public-sector establishments from the HRMIS records, `bra_hrmis`. The steps below convert the raw establishment identifiers into a canonical, well-structured establishment register according the harmonization dictionary. See below: 

```{r establishment-module}
...
```{r, echo = FALSE}
reactable(
  head(bra_hrmis_est, 10)
)
```

The key piece for harmonization is our `govhr`-specific [standard data dictionary](https://wb-pida-data-science-shop.github.io/govhr/articles/01-standard_dictionary.html) for human resources. This dictionary provides a set of variable names and definitions that set expectations on how data should be harmonized, to comply with `govhr` standards.

```{r}
head(govhr::dictionary)
```

Note that the dictionary comprises three modules: (1) establishments, (2) personnel, and (3) contract. As such, the harmonized data should comprise three tables, one for each module. Governments seldom share data in this format: the user is expected to modularize their data to conform to this modular structure.

A few `govhr` functions make this process easier (e.g., `compare_to_dictionary`), though in our experience [`dplyr`](https://dplyr.tidyverse.org/index.html) is our tool of choice. For example, suppose we are harmonizing personnel data.
...
personnel_data <- tibble::tibble(
  employee_id = c(1, 2, 3),
  time_stamp = c("2026-01-01", "2026-01-01", "2026-01-01"),
  gender = c("M", "F", "F"),
  birth_date = c("1991-26-06", "1990-20-02", "1997-12-01")
)

personnel_colnames <- govhr::dictionary |>
